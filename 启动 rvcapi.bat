...
chcp 65001 >nul
setlocal
title RVC API - MSST (Native AMD ROCm)
cd /d "%~dp0"
...
if exist "%NATIVE_PYTHON%" (
    ...
    echo [RVC] Starting MSST gateway with native AMD ROCm 7.2.1 + PyTorch 2.9.1...
    "%NATIVE_PYTHON%" "%~dp0launch.py" app_rvc.py --is_nohalf
) else if exist "%LEGACY_PYTHON%" (
    set "RVC_NATIVE_ROCM="
    echo [WARN] Native ROCm runtime not found; falling back to legacy ZLUDA...
    "%LEGACY_PYTHON%" "%~dp0launch.py" app_rvc.py --is_nohalf
) else (
