...
def _get_cache_key(song_name, model, key_shift, vocal_vol, inst_vol, reverb_intensity, delay_intensity, svc_f0_method, msst_batch_size, msst_num_overlap, msst_normalize, msst_use_tta, msst_model, uvr5_agg, uvr5_tta, uvr5_postprocess, uvr5_window_size, uvr5_high_end_process, shift_accompaniment):
    params = {
        "song": str(song_name),
        "model": str(model),
...
#               核心转换流程 & Gradio UI
# =================================================================
def convert(song_name_src, key_shift, vocal_vol, inst_vol, model_dropdown, reverb_intensity = 4, delay_intensity = 0, svc_f0_method = "fcpe", msst_batch_size = 1, msst_num_overlap = 4, msst_normalize = False, msst_use_tta = False, msst_model=DEFAULT_MODEL_ID, uvr5_agg = 10, uvr5_tta = False, uvr5_postprocess = False, uvr5_window_size = 512, uvr5_high_end_process = "mirroring", shift_accompaniment=True, progress=gr.Progress()):
    """进行翻唱推理合成"""
    ...
    cache_key = _get_cache_key(
        ...
        uvr5_high_end_process, shift_accompaniment
    )
...
with app:
    ...
    gr.Button("API Convert", visible=False).click(
        convert,
        inputs=[inp1, inp5, inp6, inp7, api_model_name, inp_reverb, inp_delay, api_svc_f0_method, api_msst_batch_size, api_msst_num_overlap, api_msst_normalize, api_msst_use_tta, api_msst_model, api_uvr5_agg, api_uvr5_tta, api_uvr5_postprocess, api_uvr5_window_size, api_uvr5_high_end_process, api_shift_accompaniment],
